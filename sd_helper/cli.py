...
@click.option("--reset", "-r", multiple=True, metavar="NAME",
              help="Reset progress for specific image (repeat for multiple); omit value to reset all")
@click.option("--reset-all", is_flag=True, help="Reset all progress")
def docker_upload_images(config_file, directory, dry_run, validate, reset, reset_all):
    ...
    import yaml
    from .docker import (
        get_image_patterns, load_progress, save_progress,
        process_image, validate_manifest,
    )
